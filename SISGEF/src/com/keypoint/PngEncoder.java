...
public class PngEncoder extends Object {
    ...
    protected int writeBytes(byte[] data, int offset) {
        ...
        if (data.length + offset > this.pngBytes.length) {
            this.pngBytes = resizeByteArray(
                this.pngBytes, this.pngBytes.length + Math.max(1000, data.length)
            );
        }
    ...
    protected int writeBytes(byte[] data, int nBytes, int offset) {
        ...
        if (nBytes + offset > this.pngBytes.length) {
            this.pngBytes = resizeByteArray(
                this.pngBytes, this.pngBytes.length + Math.max(1000, nBytes)
            );
        }
    ...
    protected void writeHeader() {
        ...
        startPos = this.bytePos = writeInt4(13, this.bytePos);
        ...
        this.bytePos = writeByte((this.encodeAlpha) ? 6 : 2, this.bytePos); // direct model
