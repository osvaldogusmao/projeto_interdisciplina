...
public class FrmValidarLancamento extends JFrame implements ActionListener {
    ...
    private JLabel lbPassivoPL;
    private JLabel lbTotalPassivoPL;
    private JLabel lbPL;
	...
	cbAnos.setBounds(160, 15, 100, cbAnos.getPreferredSize().height);
	...
	lbPassivoPL.setText("Passivo+PL");
	lbPassivoPL.setBounds(10, 100, 80, lbPassivoPL.getPreferredSize().height);
	panelComponents.add(lbPassivoPL, JLayeredPane.DEFAULT_LAYER);
	
	lbTotalPassivoPL.setText(" ");
	lbTotalPassivoPL.setFont(lbTotalPl.getFont().deriveFont(Font.BOLD));
	lbTotalPassivoPL.setForeground(new Color(146, 6, 17));
	lbTotalPassivoPL.setMaximumSize(new Dimension(lbTotalPassivoPL.getPreferredSize().height, 120));
	lbTotalPassivoPL.setHorizontalAlignment(JLabel.RIGHT);
	lbTotalPassivoPL.setBounds(90, 100, 120, lbTotalPassivoPL.getPreferredSize().height);
	panelComponents.add(lbTotalPassivoPL, JLayeredPane.DEFAULT_LAYER);
	
	lbDiferenca.setText("À Corrigir");
	lbDiferenca.setBounds(10, 120, 80, lbDiferenca.getPreferredSize().height);
	...
	lbTotalDiferenca.setBounds(90, 120, 120, lbTotalDiferenca.getPreferredSize().height);
	panelComponents.add(lbTotalDiferenca, JLayeredPane.DEFAULT_LAYER);

	lbResultado.setText("Resultado");
	lbResultado.setBounds(10, 150, 80, lbResultado.getPreferredSize().height);
	...
	lbRespostaValidador.setBounds(90, 150, 220, lbRespostaValidador.getPreferredSize().height);
	panelComponents.add(lbRespostaValidador, JLayeredPane.DEFAULT_LAYER);

	btnValidar.setText("Validar");
	btnValidar.setBounds(this.getWidth() - btnValidar.getPreferredSize().width, 170, btnValidar.getPreferredSize().width,
		btnValidar.getPreferredSize().height);
	...
	if (e.getSource() == btnValidar) {
	    ...
	    Double totalAtivo = lancamentoRepository.getTotalContaPrimeiroNivel((Integer) cbAnos.getSelectedItem(), GrupoConta.ATIVO);
	    Double totalPassivo = lancamentoRepository.getTotalContaPrimeiroNivel((Integer) cbAnos.getSelectedItem(), GrupoConta.PASSIVO);
	    Double totalPL = lancamentoRepository.getTotalContaPrimeiroNivel((Integer) cbAnos.getSelectedItem(), GrupoConta.PL);
	    ...
	    lbTotalPassivoPL.setText("(-) "+format.format(totalPassivo+totalPL));
		...
		lbRespostaValidador.setText("Verificar lançamentos");
