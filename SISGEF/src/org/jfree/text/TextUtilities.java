 ...
 * Contributor(s):   -;
 *
 * $Id: TextUtilities.java,v 1.11 2005/06/01 14:12:29 taqua Exp $
 *
 * Changes
 * -------
...
public class TextUtilities {
    ...
    protected static final LogContext logger 
        = Log.createContext(TextUtilities.class);
    ...
    public static TextBlock createTextBlock(final String text,
        ...
        return createTextBlock(
            text, font, paint, maxWidth, Integer.MAX_VALUE, measurer
        );
    }
    ...
    public static TextBlock createTextBlock(final String text,
        ...
        while (current < length && lines < maxLines) {
            final int next = nextLineBreak(
                text, current, maxWidth, iterator, measurer
            );
        ...
        if (current < length) {
            ...
            final TextFragment newFragment = new TextFragment(
                newStr, lastFragment.getFont(), lastFragment.getPaint()
            );
    ...
    /**
     * Sets the flag that controls whether or not the rotated string 
     * workaround is used.
     *
    ...
    public static Shape calculateRotatedStringBounds(final String text, 
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                "TextBoundsAnchorOffsets = " + textAdj[0] + ", " + textAdj[1]
            );   
        }
        final float[] rotateAdj = deriveRotationAnchorOffsets(
            g2, text, rotationAnchor
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                "RotationAnchorOffsets = " + rotateAdj[0] + ", " + rotateAdj[1]
            );   
        }
